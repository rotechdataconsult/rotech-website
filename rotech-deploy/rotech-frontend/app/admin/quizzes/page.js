    ...
    <AdminLayout profile={profile} title="Manage Quizzes">
            ...
            {!quiz && (
              <div className="bg-[#1E293B] border border-slate-700/50 rounded-xl p-6">
                ...
                <form onSubmit={handleCreateQuiz} className="space-y-4">
                  <div>
                    <label className="block text-xs font-medium text-slate-300 mb-1.5">Quiz Title *</label>
                    <input
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-medium text-slate-300 mb-1.5">Time (mins)</label>
                      <input
                        type="number" min="5" value={quizForm.time_limit_mins}
                        onChange={e => setQuizForm(p => ({ ...p, time_limit_mins: e.target.value }))}
                        className="w-full bg-[#0F172A] border border-slate-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-violet-500 transition"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-slate-300 mb-1.5">Pass Mark (%)</label>
                      <input
            ...
            {quiz && (
              <div className="bg-[#1E293B] border border-slate-700/50 rounded-xl p-6">
                ...
                <form onSubmit={handleAddQuestion} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {LETTERS.map(letter => (
                      <div key={letter} className="flex items-center gap-2">
                        <span className="text-xs font-bold text-violet-400 w-4 shrink-0">{letter}</span>
                        <input
                          value={qForm[`opt${letter}`]}
                          onChange={e => setQForm(p => ({ ...p, [`opt${letter}`]: e.target.value }))}
                          placeholder={`Option ${letter}`}
                          className="flex-1 bg-[#0F172A] border border-slate-700 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-violet-500 transition"
                        />
                      </div>
                    ))}
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-300 mb-1.5">Correct Answer *</label>
                    <select
                      value={qForm.correct}
                      onChange={e => setQForm(p => ({ ...p, correct: e.target.value }))}
                      className="w-full bg-[#0F172A] border border-slate-700 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-violet-500 transition cursor-pointer"
                    >
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-300 mb-1.5">Explanation (optional)</label>
                    <textarea
          ...
          <div className="space-y-3">
            ...
            {questions.length === 0 ? (
              <div className="bg-[#1E293B] border border-slate-700/50 rounded-xl p-8 text-center">
                <p className="text-slate-400 text-sm">{quiz ? 'No questions yet.' : 'Create the quiz first.'}</p>
              </div>
            ) : (
              <div className="space-y-3 max-h-[700px] overflow-y-auto pr-1">
                {questions.map((q, i) => (
                  <div key={q.id} className="bg-[#1E293B] border border-slate-700/50 rounded-xl p-4">
                    <div className="flex items-start gap-2 mb-3">
                      <span className="text-xs font-bold text-violet-400 shrink-0 mt-0.5">Q{i + 1}</span>
                    ...
                    {q.explanation && (
                      <p className="mt-2 pl-5 text-xs text-violet-400 italic">{q.explanation}</p>
                    )}
                  </div>
                ))}
              </div>
      ...
      {!selectedModule && (
        <div className="bg-[#1E293B] border border-slate-700/50 rounded-xl p-10 text-center">
          <p className="text-slate-400 text-sm">Select a domain and module to manage its quiz.</p>
        </div>
      )}
    </AdminLayout>
  )
